...
/*
 * Tests the functionality of userspace malloc and free functions.
 * During the test we repeatedly allocate and release memory using
 * malloc and free. The allocated chunks of memory must lie in the mapped
 * segment of virtual address space and must not overlap. This is tested
 * by writing chunk-specific data into an area after it has been allocated
 * and checking the consistency of the data before releasing it.
 *
...
static void test_overlap(void* addr, size_t size) {
    ...
    list_foreach(mem_blocks, mem_block_t, link, block) {
        utest_assert_fmt(!overlap_match(block, &match),
...
static void* checked_malloc(size_t size) {
    ...
    utest_check_malloc_result(data, size);
...
static void check_block(mem_block_t* blk) {
    ...
    for (pos = blk->addr, end = pos + blk->size; pos < end; pos++) {
        utest_assert_fmt(*pos == expected_value(blk, pos),
...
static mem_block_t* get_random_block(void) {
    ...
    utest_assert(block != NULL, "corrupted list of allocated memory blocks");
...
int main(void) {
    ...
    for (int phase_number = 0; phases[phase_number].name != NULL; phase_number++) {
        printf("Allocated in phase #%d: %d bytes\n", phase_number + 1,
