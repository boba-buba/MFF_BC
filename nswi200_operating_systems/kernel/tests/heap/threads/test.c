...
/*
 * Stress test for the heap and scheduler. Creates up to THREAD_COUNT
 * threads that repeatedly allocate heap memory. Checks that memory
 * is not overwritten by other threads.
 */
...
static void* worker(void* ignored) {
    ...
    while (!run_now) {
        // Spin until all workers become ready.
    }
...
void kernel_test(void) {
    ...
    for (thread_count = 0; thread_count < THREAD_COUNT; thread_count++) {
        errno_t err = thread_create(&threads[thread_count], worker, NULL, 0, "worker");
        if (err == ENOMEM) {
            break;
        }
    ...
    while (!all_started) {
        bool ipl = interrupts_disable();
        size_t thread_started_count_copy = thread_started_count;
        interrupts_restore(ipl);

        if (thread_started_count_copy == thread_count) {
            all_started = true;
        }
        thread_yield();
    }
