...
/*
 * Basic virtual memory mapping test including odd-sized address spaces.
 *
...
static void* as_worker(void* unused) {
     ...
     * not mapped.
     */
    uint8_t* data = (uint8_t*)(PAGE_NULL_COUNT * PAGE_SIZE);
    uint8_t* data_end = (uint8_t*)(size + PAGE_NULL_COUNT * PAGE_SIZE);
...
void kernel_test(void) {
    ...
    for (size_t i = PAGE_NULL_COUNT; i < MAX_PAGES; i++) {
