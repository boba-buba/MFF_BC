...
static void* as_worker(void* ignored) {
    ...
    uint8_t* data_end = (uint8_t*)((PAGE_SIZE + PAGE_NULL_COUNT) * AS_SIZE_PAGES);
...
void kernel_test(void) {
    ...
    thread_t* worker;
    ktest_thread_create_new_as_checked(&worker, as_worker, NULL, 0, "test-as-large", AS_SIZE_PAGES * PAGE_SIZE);
