 ...
 */
	...
	public:
		...
		protected:
			 ...
			 */
			void checkConstraints(const std::map<std::string, ArgBase*>& arguments) const
			{
				...
				if (isPresent()) {
					...
					for (std::set<std::string>::const_iterator it = mConflictsWith.begin(); it != mConflictsWith.end(); ++it) {
					...
					for (std::set<std::string>::const_iterator it = mRequiresAlso.begin(); it != mRequiresAlso.end(); ++it) {
						if (arguments.find(*it) == arguments.end())
							throw (ArgumentException() << "Internal Error: Argument '" << mName
		...
		protected:
			...
			bool isInt(const char* str) const
			{
				...
				while (*str && isdigit(*str))	// digits
					++str;
				if (*str == 'k' || *str == 'M' || *str == 'G' || *str == 'T')	// optional mutltiplyer
					++str;
				return !*str;
			}


			value_t processInt(int& argc, char** (&argv)) const
			{
				...
				value_t value = atol(argv[0]) * multiplier;
		...
		public:
			ArgIntBase(const std::string& name, const std::string& comment, bool mandatory = false,
				value_t min = 0, value_t max = INT64_MAX)
		...
		public:
			ArgInt(const std::string& name, const std::string& comment, bool mandatory = false,
				value_t defaultValue = 0, value_t min = 0, value_t max = INT64_MAX)
		...
		protected:
			...
			value_t processFloat(int& argc, char** (&argv))
			{
				// Check whether the argument value is present and valid.
				if (argc == 0)
					throw (ArgumentException() << "Value of argument '" << this->getName() << "' is missing!");
	...
	private:
		 ...
		 */
		std::map<std::string, ArgBase*> mArguments;
		...
		ARG_T& getArgTyped(const std::string& name)
		{
			...
			ARG_T* arg = dynamic_cast<ARG_T*>(mArguments.find(name)->second);
			if (arg == NULL)
		...
		const ARG_T& getArgTyped(const std::string& name) const
		{
			if (mArguments.find(name) == mArguments.end())
				throw (ArgumentException() << "Argument '" << name << "' was not specified.");
			const ARG_T* arg = dynamic_cast<const ARG_T*>(mArguments.find(name)->second);
			if (arg == NULL)
				throw (ArgumentException() << "Argument '" << name << "' type mismatch.");
			return *arg;
		}
	...
	public:
		...
		ProgramArguments(std::size_t namelessMin = 0, std::size_t namelessMax = ~(std::size_t)0)
			: mNamelessMin(namelessMin), mNamelessMax(namelessMax)
		{
		 ...
		 * \brief Typed accessor to argument object.
		 */
		ArgBool& getArgBool(const std::string& name)
		{
			return getArgTyped<ArgBool>(name);
		}
		 ...
		 * \brief Typed accessor to argument object.
		 */
		const ArgBool& getArgBool(const std::string& name) const
		{
			return getArgTyped<ArgBool>(name);
		}
		 ...
		 * \brief Typed accessor to argument object.
		 */
		ArgInt& getArgInt(const std::string& name)
		{
			return getArgTyped<ArgInt>(name);
		}
		 ...
		 * \brief Typed accessor to argument object.
		 */
		const ArgInt& getArgInt(const std::string& name) const
		{
			return getArgTyped<ArgInt>(name);
		}
		 ...
		 * \brief Typed accessor to argument object.
		 */
		ArgIntList& getArgIntList(const std::string& name)
		{
			return getArgTyped<ArgIntList>(name);
		}
		 ...
		 * \brief Typed accessor to argument object.
		 */
		const ArgIntList& getArgIntList(const std::string& name) const
		{
			return getArgTyped<ArgIntList>(name);
		}
		 ...
		 * \brief Typed accessor to argument object.
		 */
		ArgFloat& getArgFloat(const std::string& name)
		{
			return getArgTyped<ArgFloat>(name);
		}
		 ...
		 * \brief Typed accessor to argument object.
		 */
		const ArgFloat& getArgFloat(const std::string& name) const
		{
			return getArgTyped<ArgFloat>(name);
		}
		 ...
		 * \brief Typed accessor to argument object.
		 */
		ArgFloatList& getArgFloatList(const std::string& name)
		{
			return getArgTyped<ArgFloatList>(name);
		}
		 ...
		 * \brief Typed accessor to argument object.
		 */
		const ArgFloatList& getArgFloatList(const std::string& name) const
		{
			return getArgTyped<ArgFloatList>(name);
		}
		 ...
		 * \brief Typed accessor to argument object.
		 */
		ArgString& getArgString(const std::string& name)
		{
			return getArgTyped<ArgString>(name);
		}
		 ...
		 * \brief Typed accessor to argument object.
		 */
		const ArgString& getArgString(const std::string& name) const
		{
			return getArgTyped<ArgString>(name);
		}
		 ...
		 * \brief Typed accessor to argument object.
		 */
		ArgStringList& getArgStringList(const std::string& name)
		{
			return getArgTyped<ArgStringList>(name);
		}
		 ...
		 * \brief Typed accessor to argument object.
		 */
		const ArgStringList& getArgStringList(const std::string& name) const
		{
			return getArgTyped<ArgStringList>(name);
		}
		...
		void process(int argc, char* argv[])
		{
			...
			while (argc > 0 && argv[0] && argv[0][0] == '-') {
				// Get argument name.
				const char* name = argv[0] + 1;		// +1 ~ skip leadin dash
				--argc; ++argv;

				if (name[0] == '-') ++name;			// skip second dash too, if present.
				if (!name[0])
					break;	// empty name ~ end of named argument;

				// Find corresponding argument object.
				std::map<std::string, ArgBase*>::iterator argIt = mArguments.find(name);
			...
			for (std::map<std::string, ArgBase*>::iterator it = mArguments.begin(); it != mArguments.end(); ++it)
		...
		void printUsage(std::ostream& stream) const
		{
			...
			for (std::map<std::string, ArgBase*>::const_iterator it = mArguments.begin(); it != mArguments.end(); ++it) {
