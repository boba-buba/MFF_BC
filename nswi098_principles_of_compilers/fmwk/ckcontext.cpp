...
namespace cecko {
	...
	namespace errors {
		...
		err_def_n EMPTYCHAR{ "EMPTYCHAR", "Empty character literal" };
		err_def_n UNTERMCHAR{ "UNTERMCHAR", "Unterminated character literal" };
	...
	void context::message(errors::err_s err, loc_t loc, std::string_view msg)
	{
		...
		cov(e.name);
		out() << "Error (line " << loc << "): " << e[0] << msg << e[1] << std::endl;
	}
