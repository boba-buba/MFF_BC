...
int main()
{
    openlog("tools", LOG_CONS | LOG_PID, LOG_LOCAL0);
    syslog(LOG_INFO, "Starting the application");
    try
    {
        AllStocks stocks;
        std::size_t option = 0;
...
switchStart:
        ...
        case 1:
            {
                syslog(LOG_INFO, "case 1: Add new comapny");
        ...
        case 2:
            {
                syslog(LOG_INFO, "case 2: Calculate MA and EMA");
                std::string periods;
                std::cout << "Enter symbol of the Company\n";
        ...
        case 3:
            {
                syslog(LOG_INFO, "case 3: Calculate Volatility");
                try {
                    std::cout << "Enter symbol of the Company\n";
                    std::cin >> name; NameToUpper(name);
                    auto stock = stocks.GetStock(name);
        ...
        case 4:
            {
                syslog(LOG_INFO, "case 4: Compare two companies with correlation");
                try {
        ...
        case 5:
            {
                syslog(LOG_INFO, "case 5: Show Companies in Memory");
        ...
        case 6:
            {
                syslog(LOG_INFO, "case 6: Calculate Abnormal Returns before and after EPS was announced");
                try {
                    std::cout << "Enter symbol of the Company\n";
                    std::cin >> name; NameToUpper(name);
                    auto stock = stocks.GetStock(name);
        ...
        case 7:
            {
                syslog(LOG_INFO, "case 7: Show Company Data");
                try {
                    std::cout << "Enter symbol of the Company\n";
                    std::cin >> name; NameToUpper(name);
                    auto stock = stocks.GetStock(name);
        ...
        case 8:
            {
                syslog(LOG_INFO, "case 8: Remove Company from Memory");
                std::cout << "Enter symbol of the Company\n";
                std::cin >> name; NameToUpper(name);
    ...
    {
        syslog(LOG_ALERT, "Exception");
        std::cout << e.what() << std::endl;
    }
    syslog(LOG_INFO, "Stopping the application");
    closelog();
    return 0;
}
