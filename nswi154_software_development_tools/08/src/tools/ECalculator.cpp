...
{
	syslog(LOG_INFO, "MACalculator::calculateMA");
	vd ma;
	double sum = 0;
...
{
	syslog(LOG_INFO, "MACalculator::calculateEMA");
	vd ema;
	double sum = 0;
...
{
	syslog(LOG_INFO, "VolatilityCalculator::rangeVolatility");
	if (prices_.size() < 2)
	{
		return 0;
	}
...
{
	syslog(LOG_INFO, "VolatilityCalculator::avgDailyRange");
...
{
	syslog(LOG_INFO, "CorrelationCalculator::correlation");
	double sum = 0;
